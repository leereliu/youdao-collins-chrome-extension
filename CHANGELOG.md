...
### 新增
- 支持自定义 OpenRouter AI 模型名称。
- 新增英音和美音切换设置。
- 弹窗支持搜索历史回退。
- 链接点击增加 loading 状态。

### 改进
- 优化长文本翻译流程，有道和 AI 翻译可并行请求。
- 有道翻译失败时，已启动的 AI 翻译仍可继续展示。
- 简化链接处理逻辑，减少 overlay 闪烁。
- 使用直接导入替代动态导入，提升扩展运行稳定性。
- 删除旧版 `legacy` 目录，保留当前 Plasmo 版本作为唯一维护代码。

### 修复
- 修复 overlay 点击链接时界面闪烁的问题。
- 修复近期交互问题，提升弹窗和网页浮层的使用稳定性。

## v2.1.1

### 改进
- 将测试 fixtures 迁移为 TypeScript，保持 Vitest 测试通过。
